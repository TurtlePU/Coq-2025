...
(** We'll see some more uses of [Locate] in the second part of the lecture. *)
...
(** First, we need a type for the keys that we will use to index into
    ...
    similar purpose; here and for the rest of the course we will use the
...
(** Our main job in this chapter will be to build a definition of
    ...
    previous chapters, plus accompanying lemmas about its behavior.
...
(** **** Exercise: 2 stars, standard (t_update_same)
    ...
    [x2].  With the example in previous chapter as a template,
...
(** This property is quite useful for reasoning about languages with
    variable binding -- e.g., the Simply Typed Lambda Calculus, where maps are
...
(** We concentrate here on defining the _syntax_ and _semantics_ of
    Imp; later, in the very next lecture, we develop a theory of _program
...
(** In this chapter, we'll mostly elide the translation from the
    ...
    The optional chapter [ImpParser] of the original textbook,
    _Logical Foundations_, develops a simple lexical analyzer and parser that
    can perform this translation.  You do not need to understand that chapter to
    understand this one, but if you haven't already taken a course where these
    techniques are covered (e.g., a course on compilers) you may want to skim
    it. *)
...
(** Since we'll want to look variables up to find out their current
    values, we'll use total maps from the first part of the lesson.
...
(** In a more conventional functional programming language like OCaml or
    ...
    and workarounds (see chapter [ImpCEvalFun] of the original textbook
...
(** We'll get into more systematic and powerful techniques for
    reasoning about Imp programs in the following lecture, but we can
    already do a few things (albeit in a somewhat low-level way) just
    by working with the bare definitions.
